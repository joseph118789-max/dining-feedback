...
export default function FeedbackForm({ email }) {
  ...
  // Track in-flight requests to prevent double-submission
  ...
  const handleSubmit = async (e) => {
    ...
    // ── RACE CONDITION FIX ──────────────────────────────────────────
    // Prevent double-submission if the user clicks "Submit" multiple times.
    // If a request is already in-flight, ignore subsequent clicks.
    ...
    // AbortController lets us cancel a stale request if the component unmounts
    // or the user submits again (defence-in-depth alongside the early return above).
    const controller = new AbortController();
    submitControllerRef.current = controller;

    try {
      // Replace simulated delay with a real API call to the backend
      ...
      if (!response.ok) {
        ...
        const serverMessage = data?.error?.message || `Server error (${response.status})`;

        // Surface server-side validation errors on the matching field
        ...
        // Non-validation errors are shown as a general banner
      ...
      // Ignore abort errors — they are intentional (cancelled stale requests)
